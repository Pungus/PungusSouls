...
namespace PungusSouls
{
    ...
    public static class WeaponUpgradeRequirementTables
    {
        ...
        public static void AddLevelRequirement(string itemPrefab, int currentQuality, string requirementPrefab, int amount)
        {
            ...
            Debug.Log($"[UpgradeRequirements] Custom level found: " + $"{currentQuality}");
        ...
        private static void ApplyToRecipe(ObjectDB objectDb, Recipe recipe, string recipeItemName, RequirementTable table)
        {
            ...
            Debug.Log(
                $"[UpgradeRequirements] Applying to recipe {recipeItemName}");

            List<VRequirement> resources = new List<VRequirement>();
            ...
            {
                List<WeaponUpgradeRequirementLevel> levelRequirements = pair.Value;
                ...
                {
                    string requirementName = CleanPrefabName(levelRequirements[i].ItemPrefab);
            ...
            {
                VRequirement requirement = FindRequirement(resources, requirementName);

                if (requirement == null)
                {
                    ItemDrop itemDrop = FindItemDrop(objectDb, requirementName);

                    if (itemDrop == null)
                    {
                        Debug.LogWarning("[UpgradeRequirements] Missing requirement item '" + requirementName + "' for recipe '" + recipeItemName + "'");
                        continue;
                    }
            ...
            {
                Debug.Log(
                    $"[UpgradeRequirements] Final Resource: {resource.m_resItem.name}");
            }
        ...
        private static void Postfix(
            ...
            {
                Debug.Log(
                    $"[UpgradeRequirements] GetAmount Quality={qualityLevel}");

                if (WeaponUpgradeRequirementTables.TryGetAmount(
                    __instance,
                    qualityLevel,
                    out int amount))
                {
                    Debug.Log(
                        $"[UpgradeRequirements] Override={amount}");
